...
void render(const std::string& sceneFilePath)
{
    ...
    */
    ...
    Integrator* integrator = new RayTracerIntegrator();
    if (scene->inte_type == 0){
       // RayTracerIntegrator integrator;
       //std::cout << "raytracer integrator , purple bailu! " << std::endl;
    }else if (scene->inte_type == 1){
        // directIntegrator integrator;
        //std::cout << "direct integrator, blue  " << std::endl;
        integrator = new directIntegrator();
    }else if (scene->inte_type == 2){
        // AnalyticdirectIntegrator integrator;
        integrator = new AnalyticdirectIntegrator();
        //std::cout << "analytic direct integrator, original red  " << std::endl;
    }else if(scene->inte_type == 3){
